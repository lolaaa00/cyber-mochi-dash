...
const EndlessRunner = () => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      if (gameStateRef.current !== "playing") {
        if (e.key === "Enter" || e.key === " ") {
          if (gameStateRef.current === "menu") startGame();
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      // Determine realm
      const realm =
        REALMS[realmIndexRef.current] || REALMS[REALMS.length - 1];

      // ─── Draw Background ────────────────────────────────────
      const bgGrad = ctx.createLinearGradient(0, 0, 0, H);
      bgGrad.addColorStop(0, realm.bgGradient[0]);
      bgGrad.addColorStop(0.5, realm.bgGradient[1]);
      ...
      for (let i = 0; i < 60; i++) {
        const sx =
          ((i * 137.5 + frameRef.current * 0.1) % W);
        const sy =
          ((i * 97.3 + frameRef.current * 0.05 * (i % 3 + 1)) % (H * HORIZON_Y));
        const ss = 1 + (i % 3);
      ...
      for (let i = 0; i < 20; i++) {
        const rawZ =
          ((i * 8 - (distanceRef.current * 8) % 8 + 200) % 160);
      ...
      scoreRef.current += speedRef.current * 2;
      ...
      if (isJumpingRef.current) {
        jumpVelRef.current += 0.6; // gravity
      ...
      coinsRef.current = coinsRef.current.filter((c) => {
        const relZ = c.z - distanceRef.current;
        return relZ > -5;
      });
      ...
      for (const coin of coinsRef.current) {
        ...
        const floatY =
          Math.sin(frameRef.current * 0.05 + coin.floatPhase) * 5 * p.scale;

        if (!coin.collected) {
          // Collision with player
          const playerRelZ = 2;
          if (
            Math.abs(relZ - playerRelZ) < 3 &&
            coin.lane === targetLaneRef.current &&
            jumpRef.current > -60
          ) {
            coin.collected = true;
            if (coin.isGem) {
      ...
      obstaclesRef.current = obstaclesRef.current.filter((o) => {
        const relZ = o.z - distanceRef.current;
        return relZ > -5;
      });
      ...
      for (const obs of obstaclesRef.current) {
        ...
        // Glitch effect
        obs.glitchOffset =
          Math.sin(frameRef.current * 0.1 + obs.floatPhase) * 2 * p.scale;
        const floatY =
          Math.sin(frameRef.current * 0.04 + obs.floatPhase) * 4 * p.scale;

        // Draw obstacle avatar
        const imgKey = ["obs1", "obs2", "obs3"][obs.type];
    ...
    const drawOverlay = (ctx: CanvasRenderingContext2D, W: number, H: number) => {
      ...
      ctx.font = `bold ${Math.min(W * 0.08, 48)}px Orbitron, sans-serif`;
      ctx.textAlign = "center";
      ctx.shadowColor = "#bca2ff";
      ctx.shadowBlur = 30;
      ctx.fillText(
        isMenu ? "MOCHI RUN" : "GAME OVER",
        W / 2,
        H * 0.38
      );
      ctx.shadowBlur = 0;
      ...
      if (!isMenu) {
        // Score display
        ctx.fillStyle = "#ffffff";
        ctx.font = `${Math.min(W * 0.04, 24)}px Rajdhani, sans-serif`;
        ctx.fillText(`Score: ${Math.floor(scoreRef.current)}`, W / 2, H * 0.46);
        ctx.fillText(
          `Coins: ${coinsCountRef.current}  |  Gems: ${gemsCountRef.current}`,
          W / 2,
          H * 0.52
        );
      }
      ...
      if (mochi && mochi.complete) {
        ...
        const floatOffset =
          Math.sin(frameRef.current * 0.03) * 8;
        ctx.save();
        ctx.shadowColor = "#bca2ff";
        ctx.shadowBlur = 20;
        ctx.drawImage(
      ...
      // Controls hint
      ctx.fillStyle = "#888";
      ctx.font = `${Math.min(W * 0.03, 14)}px Rajdhani, sans-serif`;
      ctx.fillText(
        "Arrow keys / Swipe to move • Up / Swipe up to jump",
        W / 2,
        H * 0.95
      );
    };
  ...
  }, [project, spawnObstacles, spawnCoins, addParticles]);
